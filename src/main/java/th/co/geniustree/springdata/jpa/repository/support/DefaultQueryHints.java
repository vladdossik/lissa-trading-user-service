...
class DefaultQueryHints implements QueryHints {
    ...
    private QueryHints combineHints() {
        return QueryHints.from(forCounts ? metadata.getQueryHintsForCount() : metadata.getQueryHints(),
                getFetchGraphs());
    }

    private org.springframework.data.jpa.repository.support.QueryHints getFetchGraphs() {
        return Optionals.mapIfAllPresent(entityManager, metadata.getEntityGraph(),
    ...
    private JpaEntityGraph getEntityGraph(EntityGraph entityGraph) {
        ...
        String fallbackName = information.getEntityName() + "." + metadata.getMethod()
                .getName();
